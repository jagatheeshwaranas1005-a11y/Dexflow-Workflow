...
function SubmitAdView({ user, config, artists, onComplete }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const allChecked = (config?.Checklist || []).every((_, i) => checklist[`chk${i}`]);
    if (!allChecked) return alert('Please complete the checklist');
          ...
          <div className="pt-8 border-t border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {config?.Checklist?.map((item: string, i: number) => (
                <label key={i} className="flex items-center gap-3 p-4 rounded-xl bg-slate-50 border border-slate-100 hover:bg-slate-100 cursor-pointer transition-all">
                  <input 
                    type="checkbox" 
                    checked={checklist[`chk${i}`] || false}
                    onChange={e => setChecklist({ ...checklist, [`chk${i}`]: e.target.checked })}
                    className="w-5 h-5 rounded border-slate-300 text-brand-500 focus:ring-brand-500/20"
                  />
...
function MyErrorsView({ user }: any) {
  ...
  const loadErrors = async () => {
    const { data } = await supabase
      .from('audits')
      .select('*, submissions!inner(status)')
      .eq('artistName', user.name)
  ...
  const handleAppeal = async () => {
    ...
    if (!error) {
      await supabase.from('submissions').update({ status: 'Appeal Raised' }).eq('adId', selectedError?.adId);
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-4 pt-6 border-t border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(config?.Checklist || []).map((item: string, i: number) => (
                <label key={i} className="flex items-center gap-3 p-3 rounded-lg border border-slate-100 hover:bg-slate-50 transition-all cursor-pointer group">
                  <input 
                    type="checkbox" 
                    checked={checklist[`chk${i}`] || false}
                    onChange={e => setChecklist({ ...checklist, [`chk${i}`]: e.target.checked })}
                    className="w-5 h-5 rounded border-slate-300 text-brand-500 focus:ring-brand-500/20 transition-all"
                  />
  ...
  const [loading, setLoading] = useState(true);
    ...
    const fetchData = async () => {
      if (user.role === 'Artist') {
        ...
        const { data } = await supabase.from('audits').select('*').eq('prooferName', user.name).order('auditedAt', { ascending: false }).limit(50);
        if (data) setData(data);
      }
      setLoading(false);
    };
                ...
                <tr key={row.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-all">
                  ...
                  {user.role === 'Artist' ? (
                    <>
                      ...
                      <td className="px-6 py-4 text-sm font-medium text-slate-600">{row.adCategory}</td>
...
function AppealsView({ user }: any) {
  ...
  const loadAppeals = async () => {
    let query = supabase.from('appeals').select('*').order('appealedAt', { ascending: false });
    if (user.role !== 'Admin') {
  ...
  const handleResolve = async (id: number, action: string) => {
    let note = '';
    if (action === 'reject') {
      note = prompt('Optional: Rejection reason for the artist:') || '';
    }
    const status = action === 'approve' ? 'Appeal Accepted' : 'Appeal Rejected';
    const { error: appealErr } = await supabase.from('appeals').update({ status, resolutionNote: note }).eq('id', id);
    if (!appealErr) {
      const appeal = appeals.find(a => a.id === id);
      if (appeal) {
        await supabase.from('submissions').update({ status }).eq('adId', appeal.adId);
      }
      loadAppeals();
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl font-bold text-slate-900 font-display tracking-tight">Pending Appeals</h2>
        <p className="text-slate-500 mt-1 font-medium">Approve to remove the error. Reject to keep it on the artist record.</p>
      </div>
            ...
            <tbody className="divide-y divide-slate-50">
              {appeals.map(a => (
                <tr key={a.id} className="hover:bg-slate-50 transition-all group">
                  ...
                  <td className="px-4 py-4 text-slate-600 font-medium max-w-md">{a.appealDesc}</td>
                  <td className="px-4 py-4 text-slate-400 text-xs font-medium">{new Date(a.appealedAt).toLocaleString()}</td>
                  <td className="px-4 py-4">
                    {a.status === 'Pending' ? (
                      <div className="flex gap-2">
                        <Button variant="success" className="px-3 py-1.5 text-xs" onClick={() => handleResolve(a.id, 'approve')}>Approve</Button>
                        <Button variant="danger" className="px-3 py-1.5 text-xs" onClick={() => handleResolve(a.id, 'reject')}>Reject</Button>
                      </div>
              ...
              {appeals.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-12 text-center text-slate-400 font-medium italic">No pending appeals</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function AllErrorsView() {
  const [errors, setErrors] = useState<Audit[]>([]);
    ...
    const fetchAllErrors = async () => {
      const { data } = await supabase.from('audits').select('*, submissions!inner(status)').neq('errorCategory', 'No Error').order('auditedAt', { ascending: false });
      if (data) {
        setErrors(data.map((d: any) => ({ ...d, status: d.submissions?.status })));
      }
    };
    fetchAllErrors();
...
function AdminUsersView() {
  ...
  const toggleUser = async (u: User) => {
    ...
    const { error } = await supabase.from('users').update({ isActive: u.isActive ? 0 : 1 }).eq('id', u.id);
    if (!error) loadUsers();
  };
